...
def build_vocab():
    f1 = open('../insuranceQA/vocabulary')
    vocab = {}
    index = 0
    while True:
        line = f1.readline()
...
def load_vectors():
    vectors = np.loadtxt(open('../insuranceQA/wordvec.txt'))
    return vectors
...
def load_val_and_vectors():
    testList = []
    for line in open('../insuranceQA/dev_all.txt'):
        testList.append(line.strip())
    vectors = load_vectors()
    return testList, vectors
def load_test_and_vectors():
    testList = []
    for line in open('../insuranceQA/test1_all.txt'):
        testList.append(line.strip())
    return testList
