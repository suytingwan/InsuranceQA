...
class InsBiDotLSTM(object):
    def __init__(
        ...
        Wa = tf.Variable(
                tf.random_uniform([self.hidden_size*2,3], -1.0, 1.0),
                name='Wa')
        Wq = tf.Variable(
                tf.random_uniform([self.hidden_size*2,3], -1.0, 1.0),
                name='Wq')
        ...
        pool_flat_1_ = tf.reshape(pooled_1, [-1,self.hidden_size*2])
        ...
        pool_flat_2_ = tf.reshape(pooled_2, [-1,self.hidden_size82])
        '''
        for time_step in range(self.num_steps):
            cell_out = tf.reshape(outputs_1_[time_step],[-1,self.hidden_size*2])
            att = tf.reduce_sum(cell_out * last_output_2, 1)
            att = tf.reshape(att,[-1,1])
            att_q.append(att)

        s_att_q = tf.concat(1, att_q)
        s_att_q = tf.reshape(s_att_q, [-1,self.num_steps,1])
        output_att_1 = trans_out_1 * s_att_q
        pool_reshape_1 = tf.reshape(output_att_1, [-1, self.num_steps, self.hidden_size*2, 1])
        pooled_1 = tf.nn.max_pool(
                pool_reshape_1,
                ksize=[1,self.num_steps,1,1],
                strides=[1,1,1,1],
